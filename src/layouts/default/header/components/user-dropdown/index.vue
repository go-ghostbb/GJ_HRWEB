<template>
  <Dropdown placement="bottomLeft" :overlayClassName="`${prefixCls}-dropdown-overlay`">
    <span :class="[prefixCls, `${prefixCls}--${theme}`]" class="flex">
      <img :class="`${prefixCls}__header`" :src="getEmployeeInfo.avatar" />
      <span :class="`${prefixCls}__info hidden md:block`">
        <span :class="`${prefixCls}__name`" class="truncate">
          {{ getEmployeeInfo.realName }}
        </span>
      </span>
    </span>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'EmployeeDropdown' });

  defineProps({
    theme: propTypes.oneOf(['dark', 'light']),
  });

  const { prefixCls } = useDesign('header-employee-dropdown');
  const { t } = useI18n();
  const { getShowDoc, getUseLockPage, getShowApi } = useHeaderSetting();
  const employeeStore = useEmployeeStore();

  const getEmployeeInfo = computed(() => {
    const { realName = '', avatar, desc } = employeeStore.getEmployeeInfo || {};
...
<style lang="less">
  @prefix-cls: ~'@{namespace}-header-employee-dropdown';
